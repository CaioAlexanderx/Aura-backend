...
// Cria uma venda completa de forma atômica
// Body: { items, customer_id?, payment_method, discount_amount?,
//         discount_pct?, notes?, seller_id?, payments? }
// items: [{ product_id, variant_id?, quantity, unit_price,
//           product_name_snapshot? }]
// payments: [{ method, amount }] (múltiplos pagamentos)
// ============================================================
router.post('/sale', guard, async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      // Busca snapshot do produto (nome, custo)
    ...
    // ── 4. Inserir itens + baixa de estoque ───────────────────
      ...
      if (item.product_id) {
        ...
        await client.query(
          `INSERT INTO stock_movements
             (product_id, company_id, type, quantity, reference_id,
              reference_type, notes)
           VALUES ($1,$2,'out',$3,$4,'sale','Venda PDV')
    ...
    // ── 5. Múltiplos pagamentos (opcional) ──────────────────
    if (payments?.length > 1) {
      for (const p of payments) {
        await client.query(
          `INSERT INTO sale_payments (sale_id, company_id, method, amount)
           VALUES ($1,$2,$3,$4)
           ON CONFLICT DO NOTHING`,
    ...
    // ── 6. Atualizar métricas do cliente (trigger já faz isso, é redundante mas garante) ─
    if (customer_id) {
      await client.query(
  ...
  try {
    ...
    const { rows: items } = await db.query(
      `SELECT si.*, COALESCE(p.name, si.product_name_snapshot) AS product_name
       FROM sale_items si LEFT JOIN products p ON p.id=si.product_id
       WHERE si.sale_id=$1`, [req.params.saleId]
    );
    res.json({ ...rows[0], items });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// GET /companies/:id/pdv/sales?date=YYYY-MM-DD&limit=50
  ...
  try {
    const { rows } = await db.query(
      `SELECT s.id, s.total_amount, s.discount_amount, s.payment_method,
              s.created_at, u.full_name AS seller_name, c.name AS customer_name,
...
// GET /companies/:id/pdv/summary?date=YYYY-MM-DD
// Resumo do dia: total vendido, ticket médio, formas de pagamento
  ...
  try {
    const { rows } = await db.query(
      `SELECT
       ...
       FROM (
         SELECT s.total_amount, s.discount_amount, s.payment_method,
                COUNT(*) OVER (PARTITION BY s.payment_method) AS cnt
         FROM sales s
         WHERE s.company_id=$1 AND s.created_at::date=$2
       ) sub`,
      [req.params.id, date]
    );
...
router.delete('/sale/:saleId', guard, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE sales SET notes=CONCAT(COALESCE(notes,''),' [CANCELADA]'), updated_at=NOW() WHERE id=$1`,
      [req.params.saleId]
    );
    await client.query('COMMIT');
