// ============================================================
// AURA. -- Product Categories CRUD
// Gerencia categorias cadastradas por empresa.
// Produtos continuam gravando `category` como texto para compat;
// ao renomear, cascade update nos produtos; ao excluir, decide se
// move produtos para outra categoria ou deixa o texto como esta.
// ============================================================
...
router.get('/', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `SELECT
         c.id, c.name, c.color, c.sort_order, c.created_at, c.updated_at,
         (SELECT COUNT(*)::int FROM products p
            WHERE p.company_id = c.company_id AND p.category = c.name) AS product_count
...
router.post('/', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO product_categories (company_id, name, color, sort_order)
       VALUES ($1, $2, $3, $4)
       RETURNING id, name, color, sort_order, created_at`,
      [cid, cleanName, hex, parseInt(sort_order) || 0]
    );
...
// PATCH /product-categories/:cid
// Se name mudar, cascade update em products.category
router.patch('/:catId', async (req, res) => {
  ...
  try {
    ...
    const { rows: existingRows } = await client.query(
      'SELECT name FROM product_categories WHERE id = $1 AND company_id = $2 FOR UPDATE',
      [catId, cid]
    );
    ...
    const { rows } = await client.query(
      `UPDATE product_categories SET ${updates.join(', ')}
       ...
       RETURNING id, name, color, sort_order, updated_at`,
      values
    );

    // Cascade: se o nome mudou, atualiza products.category
...
// Query ?move_to=NomeCategoria move os produtos para outra categoria
// antes de remover. Sem move_to, produtos ficam com o texto antigo
// (nao cascade no texto; texto passa a apontar para categoria inexistente).
  ...
  try {
    ...
    if (moveTo) {
      ...
      const { rows: destRows } = await client.query(
        'SELECT name FROM product_categories WHERE company_id = $1 AND name = $2',
        [cid, moveTo]
      );
      if (!destRows.length) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'Categoria destino nao encontrada' });
      }
      const upRes = await client.query(
        `UPDATE products SET category = $1, updated_at = NOW()
         WHERE company_id = $2 AND category = $3`,
    ...
    await client.query('COMMIT');
    res.json({ deleted: true, id: catId, moved_products: movedProducts });
  } catch (err) {
    await client.query('ROLLBACK');
