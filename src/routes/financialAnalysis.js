// ============================================================
// AURA. — Analise Financeira v3
// FIX: Labels em portugues, insights acionaveis
// ============================================================
var router = require('express').Router({ mergeParams: true });
var db = require('../config/database');
var { requireAuth } = require('../middleware/auth');

// PT-BR month labels (Postgres returns English)
...
router.get('/', requireAuth, async function(req, res) {
  ...
  var numMonths = Math.min(Math.max(parseInt(req.query.months) || 13, 3), 24);
  ...
  try {
    var monthly = (await db.query(
      ...
      " GROUP BY month, label ORDER BY month ASC", [cid, numMonths])).rows;
    ...
    var currentMonth = (await db.query(
      ...
      " AND created_at < date_trunc('month',NOW())+INTERVAL '1 month'", [cid])).rows;
    ...
    var prevMonth = (await db.query(
      ...
      " AND created_at < date_trunc('month',NOW())", [cid])).rows;
    ...
    try {
      employees = (await db.query(
        ...
        " ) combined GROUP BY name ORDER BY faturamento DESC", [cid, numMonths])).rows;
    } catch (_) {}
    ...
    try {
      topCustomers = (await db.query(
        ...
        " ORDER BY total_gasto DESC LIMIT 20", [cid, numMonths])).rows;
    } catch (_) {}
    ...
    var ticketDist = (await db.query(
      ...
      " FROM transactions WHERE company_id=$1 AND type='income'" +
      " AND created_at >= date_trunc('month',NOW())-(($2::int-1)||' months')::interval" +
      " GROUP BY faixa ORDER BY MIN(amount)", [cid, numMonths])).rows;
    ...
    var completedMonths = monthly.filter(function(m) { return new Date(m.month + '-01') < new Date(new Date().getFullYear(), new Date().getMonth(), 1); });
    ...
    // Find best/worst day (using DOW number, convert to PT-BR)
    ...
    res.json({
      monthly: monthly.map(function(m) {
        return {
          month: m.month, label: labelPtBR(m.label),
      ...
      insights: {
        ...
        best_month: bestMonth ? { month: bestMonth.month, label: labelPtBR(bestMonth.label), receita: parseFloat(bestMonth.receita) } : null,
        worst_month: worstMonth ? { month: worstMonth.month, label: labelPtBR(worstMonth.label), receita: parseFloat(worstMonth.receita) } : null,
