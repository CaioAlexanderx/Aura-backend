...
router.post('/import-ofx', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (const tx of toSave) {
      // ⚠️ O `WHERE fitid IS NOT NULL` NÃO É ENFEITE — repete o predicado do
      // índice PARCIAL que garante a deduplicação por FITID:
      //
      //   idx_transactions_fitid_company ON transactions (company_id, fitid)
      //     WHERE fitid IS NOT NULL
      //
      // (parcial de propósito: a esmagadora maioria das transactions não vem
      // de extrato e tem fitid NULL — um índice total desperdiçaria a tabela
      // inteira e ainda trataria os NULLs como distintos.)
      //
