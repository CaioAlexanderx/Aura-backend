...
router.get('/', async function(req, res) {
  ...
  try {
    ...
    var dataRes = await db.query(
      'SELECT id, type, amount, description, category, status, notes, due_date, paid_at, created_at, recurrence_type, recurrence_group_id, recurrence_index' +
...
router.post('/', async function(req, res) {
  ...
  try {
    if (!recurrenceType) {
      ...
      return res.status(201).json({ id: tx.id, type: tx.type, amount: parseFloat(tx.amount), description: tx.description, category: tx.category, status: tx.status, due_date: tx.due_date, paid_at: tx.paid_at, created_at: tx.created_at });
    }
    ...
    for (var i = 0; i < recurrenceCount; i++) {
      ...
      var r = await db.query(
        'INSERT INTO transactions (company_id, type, amount, description, category, notes, due_date, status, paid_at, created_by, recurrence_type, recurrence_group_id, recurrence_index)' +
        ' VALUES ($1, $2, $3, $4, $5, $6, $7, $8, ' + itemPaidAt + ', $9, $10, $11, $12)' +
