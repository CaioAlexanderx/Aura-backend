...
// Default status: confirmed (registro de algo que ja aconteceu)
// Usar pending somente para receitas/despesas a receber/pagar
// ============================================================
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const params = [cid];
    if (type === 'income' || type === 'expense') { where += ` AND type = $${params.length + 1}`; params.push(type); }
    ...
    const sumRes = await db.query(
      `SELECT COALESCE(SUM(CASE WHEN type='income' THEN amount ELSE 0 END), 0) AS income,
       ...
       FROM transactions WHERE company_id = $1 AND created_at >= date_trunc('month', CURRENT_DATE)`, [cid]
    );
    ...
    res.json({
      transactions, total: parseInt(countRes.rows[0]?.total) || 0, limit, offset,
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await db.query(
      ...
      [cid, type, parseFloat(amount), String(description).trim(), category || 'Outros',
       notes || null, due_date || new Date().toISOString().slice(0, 10), finalStatus, req.user?.id || null]
    );
    const tx = result.rows[0];
    res.status(201).json({
      id: tx.id, type: tx.type, amount: parseFloat(tx.amount),
      description: tx.description, category: tx.category,
      status: tx.status, paid_at: tx.paid_at, created_at: tx.created_at,
    });
...
router.patch('/:txId', async (req, res) => {
  ...
  const fields = ['type', 'amount', 'description', 'category', 'status', 'notes', 'due_date'];
...
router.delete('/:txId', async (req, res) => {
  const { id: cid, txId } = req.params;
  try {
    const result = await db.query('DELETE FROM transactions WHERE id = $1 AND company_id = $2 RETURNING id', [txId, cid]);
    if (!result.rows.length) return res.status(404).json({ error: 'Lancamento nao encontrado' });
