// ============================================================
// AURA. — INT-COMPANY-01: Company Profile CRUD
...
// GET /profile — return full company profile
router.get('/profile', async (req, res) => {
  try {
    const { rows } = await db.query(
      `SELECT id, trade_name, legal_name, cnpj, phone, email, address,
              logo_url, tax_regime, business_type, plan, billing_status,
...
router.put('/profile', async (req, res) => {
  ...
  for (const [bodyKey, dbCol] of Object.entries(allowedFields)) {
    if (req.body[bodyKey] !== undefined && !seen.has(dbCol)) {
      ...
      if (dbCol === 'tax_regime' && req.body[bodyKey]) {
        const valid = ['mei', 'simples', 'lucro_presumido', 'lucro_real'];
  ...
  try {
    ...
    console.error('[company] profile PUT error:', err.message);
