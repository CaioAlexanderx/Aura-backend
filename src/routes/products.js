...
router.get('/:pid/variants', async (req, res) => {
  ...
  try {
    const { rows: check } = await db.query('SELECT id FROM products WHERE id=$1 AND company_id=$2', [productId, companyId]);
    ...
    const { rows } = await db.query(`
      ...
      GROUP BY pv.id, pv.sku_suffix, pv.price_override, pv.stock_qty, pv.barcode ORDER BY pv.created_at ASC
    `, [productId]);
