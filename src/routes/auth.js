...
function setRefreshCookie(res, refreshToken) {
  res.cookie('aura_refresh', refreshToken, {
    ...
    maxAge: REFRESH_TTL_MS, path: '/api/v1/auth',
  });
}

function clearRefreshCookie(res) {
  res.clearCookie('aura_refresh', { path: '/api/v1/auth' });
}
  ...
  try {
    await db.query(
      `INSERT INTO refresh_tokens (user_id, token_hash, expires_at, ip_address, user_agent)
       VALUES ($1, $2, $3, $4, $5)`,
      [userId, hashToken(refreshToken), new Date(Date.now() + REFRESH_TTL_MS),
  ...
  try {
    ...
    const { rows: [company] } = await client.query(
      `INSERT INTO companies (owner_id, legal_name, trade_name, plan, onboarding_step, trial_ends_at, access_code_used, cnpj)
       ...
       RETURNING id, legal_name, trade_name, plan, onboarding_step, trial_ends_at, module_overrides`,
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user.totp_enabled) {
      return res.json({ requires_2fa: true, user_id: user.id, message: 'Autenticacao de dois fatores necessaria. Envie o codigo para /auth/2fa/validate' });
    }
...
router.post('/refresh', async (req, res) => {
  ...
  try {
    ...
    try {
      const { rows } = await db.query(
        `SELECT id, revoked FROM refresh_tokens WHERE token_hash = $1 AND user_id = $2`, [tokenHash, decoded.id]
      );
      if (rows.length > 0 && rows[0].revoked) {
        clearRefreshCookie(res);
        return res.status(401).json({ error: 'Refresh token revogado', code: 'REFRESH_REVOKED' });
      }
    } catch (_) {}

    const { rows } = await db.query(
      `SELECT u.id, u.role, u.is_staff, c.id AS company_id, c.plan
       FROM users u
       LEFT JOIN company_members cm ON cm.user_id = u.id AND cm.status = 'active' AND cm.is_active = true
       LEFT JOIN companies c ON c.id = cm.company_id
...
// ── POST /api/v1/auth/logout (SEC-02 + SEC-06) ───────────
router.post('/logout', async (req, res) => {
  const refreshTokenInput = req.body.refresh_token || req.cookies?.aura_refresh;
  if (!refreshTokenInput) return res.json({ message: 'Logout realizado' });

  try {
    const decoded = jwt.verify(refreshTokenInput, JWT_SECRET, { ignoreExpiration: true });
    const tokenHash = hashToken(refreshTokenInput);
    try {
      await db.query(`UPDATE refresh_tokens SET revoked = true, revoked_at = NOW() WHERE token_hash = $1 AND user_id = $2`, [tokenHash, decoded.id]);
    } catch (_) {}
