// ============================================================
// AURA. — Food Service
// Cardapio por periodo — horarios de ativacao automatica
// (rotas AUTENTICADAS — montadas em /companies/:id/food/schedule)
// ============================================================
// A rota publica /active foi movida para foodSchedulePublic.js,
// porque o mesmo arquivo era montado em /food/schedule (sem auth)
// e em /companies/:id/food/schedule (autenticado).
// ============================================================
...
const notFound = (res, e='Horario') => res.status(404).json({ error: `${e} nao encontrado` });
...
router.get('/', guard, async (req, res) => {
  try {
    ...
    res.json(result);
  } catch (e) { console.error('[food/schedule]', e.message); res.status(500).json({ error: 'Erro ao buscar horarios' }); }
});

router.post('/', guard, async (req, res) => {
  const { menu_id, label, days_of_week, start_time, end_time } = req.body;
  if (!menu_id || !label || !start_time || !end_time)
    return res.status(400).json({ error: 'menu_id, label, start_time e end_time obrigatorios' });
  const { rows: m } = await db.query(
    `SELECT id FROM food_menus WHERE id=$1 AND company_id=$2`, [menu_id, req.params.id]
  );
  if (!m.length) return notFound(res, 'Cardapio');
  try {
  ...
  } catch (e) { console.error('[food/schedule]', e.message); res.status(500).json({ error: 'Erro ao criar horario' }); }
});
...
router.patch('/:sid', guard, async (req, res) => {
  ...
  try {
    ...
    res.json(rows[0]);
  } catch (e) { console.error('[food/schedule]', e.message); res.status(500).json({ error: 'Erro ao atualizar horario' }); }
});
...
router.delete('/:sid', guard, async (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e) { console.error('[food/schedule]', e.message); res.status(500).json({ error: 'Erro ao remover horario' }); }
});

router.patch('/menu/:mid/always-available', guard, async (req, res) => {
  const { always_available } = req.body;
  if (always_available === undefined)
    return res.status(400).json({ error: 'always_available obrigatorio' });
  try {
    const { rows } = await db.query(
    ...
    if (!rows.length) return notFound(res, 'Cardapio');
    res.json(rows[0]);
  } catch (e) { console.error('[food/schedule]', e.message); res.status(500).json({ error: 'Erro ao atualizar cardapio' }); }
});
