...
// ODT-14 (23/04): D-UNIFY + status PT-BR + TZ SP
//   - pacientes_recall: customers is_patient=true + a.customer_id
//   - consultas_hoje: status PT-BR (confirmado/agendado/faltou) + TZ SP
//   - parcelas_vencidas: TZ SP em CURRENT_DATE
//   - procedimentos_mes: status 'concluido'
//   - price_total (nao final_price, col correta)
// ============================================================
...
async function getContextData(companyId, context) {
  ...
  try {
    switch (context) {
      case 'financeiro': {
        const { rows: [bal] } = await db.query(
          `SELECT
           ...
           FROM transactions
           WHERE company_id=$1
             AND (due_date AT TIME ZONE 'America/Sao_Paulo') >= date_trunc('month', (NOW() AT TIME ZONE 'America/Sao_Paulo'))`,
          [companyId]);
        ...
        const { rows: overdue } = await db.query(
          `SELECT description, amount, due_date FROM transactions
           WHERE company_id=$1 AND type='income' AND status='pending'
             AND due_date < (NOW() AT TIME ZONE 'America/Sao_Paulo')::date
           ORDER BY amount DESC LIMIT 5`,
          [companyId]);
        data.contas_a_receber_vencidas = overdue.map(r => ({
          desc: r.description, valor: parseFloat(r.amount), venc: r.due_date,
        }));
        break;
      }
      case 'estoque': {
        ...
        data.estoque_baixo = low.map(r => ({
          nome: r.name, atual: parseInt(r.stock_qty), min: parseInt(r.stock_min), preco: parseFloat(r.price),
        }));
        ...
        const { rows: top } = await db.query(
          `SELECT p.name, COALESCE(SUM(si.quantity), 0) AS vendidos
           ...
           GROUP BY p.name ORDER BY vendidos DESC LIMIT 5`,
          [companyId]);
      ...
      case 'crm': {
        ...
        data.clientes_inativos_30d = inactive.map(r => ({
          nome: r.name, telefone: r.phone,
          ultima_compra: r.last_purchase_at, total_gasto: parseFloat(r.total_spent),
        }));
      ...
      case 'marketing': {
        const { rows: trend } = await db.query(
          `SELECT DATE(created_at) AS dia, COUNT(*) AS vendas, SUM(total_amount) AS valor
           FROM sales WHERE company_id=$1 AND status != 'cancelled'
             AND created_at >= NOW() - INTERVAL '14 days'
           GROUP BY DATE(created_at) ORDER BY dia`, [companyId]);
        data.tendencia_vendas_14d = trend.map(r => ({
          dia: r.dia, vendas: parseInt(r.vendas), valor: parseFloat(r.valor),
        }));
      ...
      case 'odonto': {
        ...
        try {
          const { rows: [ov] } = await db.query(
            ...
            [companyId]);
          data.parcelas_vencidas = {
            qtd:   parseInt(ov?.cnt     || 0),
            valor: parseFloat(ov?.valor || 0),
          };
        } catch (_) { data.parcelas_vencidas = { qtd: 0, valor: 0 }; }

        // Pacientes para recall (D-UNIFY: customers is_patient=true + customer_id)
        try {
        ...
        try {
          const { rows: procs } = await db.query(
            ...
            [companyId]);
          data.procedimentos_mes = procs.map(r => ({
            nome: r.procedure_name, qtd: parseInt(r.qtd), receita: parseFloat(r.receita),
          }));
    ...
    console.error(`[aiContext] Error ${context}:`, err.message);
  }
  return data;
}
