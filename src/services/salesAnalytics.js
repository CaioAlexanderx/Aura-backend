...
//
// REVISAO 02/06/2026 (troca pelo LIQUIDO):
//   Antes a troca era EXCLUIDA dos agregados (type='troca' filtrado fora),
//   porque sales.total_amount da troca = newValue (produto novo) inflava.
//   Agora a troca CONTA pelo LIQUIDO: total_amount - SUM(troca_returned_items).
//   Alinhado com sales.js (tela /vendas). Faturamento passa a refletir o
//   caixa real da troca (diferenca recebida), nem cheio nem zero.
//   total_sales/avg_ticket continuam sobre vendas normais (troca nao e
//   "ticket"). top_products/top_employees sem filtro (atribuicao fisica).
//
// REVISAO 09/05/2026 (Eryca Finesse — fonte unica de vendas):
//   total_revenue vem de SALES.total_amount (status != cancelled).
//   Alinha KPI "Vendas hoje" do Painel + stats.revenue da tela /vendas.
//
...
// Expressao SQL do valor LIQUIDO da venda no agregado de faturamento:
//   venda normal -> total_amount
//   troca        -> total_amount - SUM(troca_returned_items.qty*unit_price)
// Requer que a query rode com FROM sales (correlaciona em sales.id).
const NET_REVENUE_EXPR =
  "(CASE WHEN COALESCE(sales.type,'sale') = 'troca' " +
  "      THEN sales.total_amount - (SELECT COALESCE(SUM(tri.quantity*tri.unit_price),0) " +
  "                                   FROM troca_returned_items tri WHERE tri.troca_sale_id = sales.id) " +
  "      ELSE sales.total_amount END)";

// Calcula data SP sem depender de ICU/locale (SP = UTC-3 fixo).
function todaySP() {
...
/**
 * Resumo do periodo.
 * 02/06/2026: total_revenue conta troca pelo LIQUIDO; total_sales/avg_ticket
 * sobre vendas normais (troca nao e ticket).
 */
async function getSummary(companyId, startDate, endDate) {
  const SP = `AT TIME ZONE 'America/Sao_Paulo'`;
  const NOT_CANCELLED = "COALESCE(status,'completed') != 'cancelled'";
  const IS_SALE = "COALESCE(type,'sale') = 'sale'";

  const { rows } = await db.query(`
    SELECT
      COUNT(*)::int                                                                                AS total_sales_all,
      COUNT(*) FILTER (WHERE ${NOT_CANCELLED} AND ${IS_SALE})::int                                 AS total_sales,
      COALESCE(SUM(${NET_REVENUE_EXPR}) FILTER (WHERE ${NOT_CANCELLED}), 0)                         AS total_revenue,
      COALESCE(SUM(total_amount) FILTER (WHERE ${NOT_CANCELLED} AND ${IS_SALE}), 0)                 AS sales_revenue,
      COALESCE(SUM(discount_amount) FILTER (WHERE ${NOT_CANCELLED}), 0)                             AS total_discounts,
      COUNT(DISTINCT customer_id) FILTER (WHERE ${NOT_CANCELLED})::int                              AS unique_customers,
      COUNT(DISTINCT (created_at ${SP})::date) FILTER (WHERE ${NOT_CANCELLED})::int                 AS active_days
    FROM sales
    WHERE company_id = $1
  ...
  const totalRevenue = parseFloat(row.total_revenue) || 0;   // faturamento liquido (vendas + troca net)
  const salesRevenue = parseFloat(row.sales_revenue) || 0;   // so vendas normais (pra avg ticket)
  const totalSales   = parseInt(row.total_sales)     || 0;
  const avgTicket    = totalSales > 0
    ? parseFloat((salesRevenue / totalSales).toFixed(2))
    : 0;
...
/**
 * Serie temporal. total_revenue por periodo conta troca pelo LIQUIDO.
 */
async function getTimeSeries(companyId, startDate, endDate, groupBy) {
  ...
  const { rows } = await db.query(`
    SELECT
      ${fmt} AS period,
      COUNT(*) FILTER (WHERE COALESCE(sales.type,'sale')='sale')::int AS total_sales,
      COALESCE(SUM(${NET_REVENUE_EXPR}), 0)                          AS total_revenue
    FROM sales
    WHERE company_id = $1
 ...
 * Sem filtro de type — produtos vendidos em troca sao saida fisica real.
 */
...
/**
 * Ranking de funcionarios — sales. Sem filtro de type.
 */
...
/**
 * Vendas por metodo de pagamento — conta troca pelo LIQUIDO.
 */
