...
| **DEC-08** | **Importação com categoria fora da árvore deixa o produto PENDENTE no wizard, não cria categoria.** Nome ambíguo (mesmo nome em dois ramos) também fica pendente. | Planilha suja não vira árvore suja. O texto **continua** em `products.category`, porque é a chave de junção do wizard (`applyCategoryRow` casa por `products.category = raw_value`) — apagá-lo perderia os produtos de vista. "Pendente" = linha em `category_migration_staging`, não texto apagado. Ver D4 em `F0_EXECUCAO.md`. |
