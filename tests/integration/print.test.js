...
const mockCompany = { legal_name:'Mercado do João', trade_name:'Mercado João', cnpj:'12.345.678/0001-90', phone:'(12)99999-9999' };
const mockSale    = { id:saleId, total_amount:85.50, discount_amount:0, payment_method:'Pix', created_at: new Date().toISOString(), seller_name:'Ana', customer_name:'Carlos' };
const mockItems   = [
  { product_name:'Feijão 1kg', quantity:2, unit_price:7.50, total_price:15.00 },
  { product_name:'Arroz 5kg',  quantity:1, unit_price:25.00, total_price:25.00 },
];
...
describe('GET /print/receipt/:saleId', () => {
  ...
  test('retorna HTML com Content-Type text/html', async () => {
    db.query.mockResolvedValueOnce({ rows: [mockCompany] }).mockResolvedValueOnce({ rows: [mockSale] }).mockResolvedValueOnce({ rows: mockItems });
  ...
  test('retorna 404 se venda não encontrada', async () => {
    db.query.mockResolvedValueOnce({ rows: [mockCompany] }).mockResolvedValueOnce({ rows: [] });
    const res = await request(app).get(`/api/v1/companies/${cid}/print/receipt/${saleId}`).set(auth);
