...
describe('GET /members', () => {
  ...
  test('monthly_cost=0 com 1 membro (titular)', async () => {
    db.query.mockResolvedValueOnce({ rows: [{ id:'m1', status:'active', is_active:true }] });
    const res = await request(app).get(`/api/v1/companies/${cid}/members`).set(auth);
    expect(res.body.monthly_cost).toBe(0);
  });

  test('monthly_cost=19 com 2 membros ativos', async () => {
    db.query.mockResolvedValueOnce({ rows: [{ id:'m1', status:'active', is_active:true }, { id:'m2', status:'active', is_active:true }] });
    const res = await request(app).get(`/api/v1/companies/${cid}/members`).set(auth);
...
describe('POST /members/invite', () => {
  ...
  test('retorna 400 sem invite_email', async () => {
    const res = await request(app).post(`/api/v1/companies/${cid}/members/invite`).set(auth).send({ role_label:'vendedor' });
  ...
  test('retorna 409 se email já tem convite', async () => {
    ...
    const res = await request(app).post(`/api/v1/companies/${cid}/members/invite`).set(auth).send({ invite_email:'joao@teste.com' });
  ...
  test('cria convite com sucesso', async () => {
    db.query
      ...
      .mockResolvedValueOnce({ rows: [] })
      .mockResolvedValueOnce({ rows: [{ id:'m2', invite_token:'tok', invite_email:'joao@teste.com', status:'pending' }] });
    const res = await request(app).post(`/api/v1/companies/${cid}/members/invite`).set(auth).send({ invite_email:'joao@teste.com', role_label:'vendedor' });
...
describe('GET /members/roles', () => {
  ...
  test('retorna templates', async () => {
    db.query.mockResolvedValueOnce({ rows: [{ id:'r1', name:'Vendedor', is_default:true, type:'global' }] });
